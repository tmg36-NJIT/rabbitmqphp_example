...
function doLoginDB($username, $password) {
	...
	if(!$row){return ['success' => false, 'message' => "Invalid username and/or password"];}

	if(!password_verify($password, $row['password'])) {return ['success' => false, 'message' => "Invalid username and/or password"];}

	$sessionKey = createSession($row['id']);
	if ($sessionKey) {return ['success' => true, 'message' => "Login successful", 'session_key' => $sessionKey];
...
function requestProcessor($request) {
	...
	switch($request['type']){
		...
		if ($query === '') { return ['success' => false, 'message' => 'Empty search query']; }
	 	$ok = saveUserSearch($username, $query);
